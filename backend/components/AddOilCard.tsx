...
const AddOilCard = ({ show, setShow }: any) => {
  ...
  const submit = async (e: { preventDefault: () => void }) => {
    ...
    setUploading(true);
    try {
      const fetcher = await fetch("/api/add_oil", {
        method: "POST",
        body: JSON.stringify({
          name: form?.name,
          litre: form?.litre,
      ...
      {show !== 0 && (
        <div className="fixed top-0 left-0 z-50 flex items-center justify-center w-full h-full py-5 bg-gray-200 bg-opacity-50">
          <div className="relative w-full max-w-3xl max-h-full px-10 py-10 mx-auto overflow-y-auto text-black bg-white rounded-lg shadow-2xl no_scrollbar 2xl:max-w-5xl">
            <h3 className="mb-5 font-bold">Add Oil</h3>
            <button
              disabled={uploading}
              type="button"
              onClick={() => {
            ...
            <form onSubmit={submit}>
              <h5 className="mb-2 font-semibold">Oil Name</h5>
              <input
                type="text"
                placeholder="..."
                ...
                autoComplete="off"
              />
              <h5 className="mb-2 font-semibold">Oil Litre</h5>
